...
def load_data(datafile, flare_label, series_len, start_feature, n_features, mask_value):
    ...
    for idx in range(0, len(df_values)):
        ...
        row = df_values[idx]
        label = row[1][0]
        if flare_label == 'M5' and row[0][0] == 'M' and float(row[0][1:]) >= 5.0:
            label = 'X'
        ...
        if has_zero_record is False:
            ...
            if len(each_series_data) > 0:
                X.append(np.array(each_series_data).reshape(
                    series_len, n_features).tolist())
                y.append(label)
    X_arr = np.array(X)
    y_arr = np.array(y)
    print(type(y_arr))
...
def attention_3d_block(hidden_states, series_len):
    ...
    hidden_states_t = Reshape(
        (hidden_size, series_len), name='attention_input_reshape')(hidden_states_t)
    score_first_part = Dense(series_len, use_bias=False,
                             name='attention_score_vec')(hidden_states_t)
    score_first_part_t = Permute(
        (2, 1), name='attention_score_vec_t')(score_first_part)
    h_t = Lambda(lambda x: x[:, :, -1], output_shape=(hidden_size,
                 1), name='last_hidden_state')(hidden_states_t)
    score = dot([score_first_part_t, h_t], [2, 1], name='attention_score')
    attention_weights = Activation('softmax', name='attention_weight')(score)
    context_vector = dot([hidden_states_t, attention_weights], [
                         2, 1], name='context_vector')
    context_vector = Reshape((hidden_size,))(context_vector)
    h_t = Reshape((hidden_size,))(h_t)
    pre_activation = concatenate(
        [context_vector, h_t], name='attention_output')
    attention_vector = Dense(hidden_size, use_bias=False,
...
def lstm(nclass, n_features, series_len):
    ...
    output = Dense(nclass, activation='sigmoid',
                   activity_regularizer=regularizers.l2(0.0001))(layer2_out)
...
if __name__ == '__main__':
    ...
    if train_again == 1:
        # Train
        X_train_data, y_train_data = load_data(datafile=filepath + 'training.csv',
        ...
        history = model.fit(X_train, y_train_tr,
                            ...
                            verbose=True, shuffle=True, class_weight=class_weight_)
        model.save('./model.h5')
    else:
        model = load_model('./model.h5')

    # Test
    X_test_data, y_test_data = load_data(datafile=filepath + 'testing.csv',
                                         flare_label=flare_label, series_len=series_len,
                                         start_feature=start_feature, n_features=n_features,
                                         mask_value=mask_value)
    ...
    classes = model.predict(
        X_test, batch_size=batch_size, verbose=0, steps=None)

    with open(result_file, 'w', encoding='UTF-8') as result_csv:
        w = csv.writer(result_csv)
        with open(filepath + 'testing.csv', encoding='UTF-8') as data_csv:
